...
class TripForm extends React.Component {
  ...
  componentDidMount() {
    const { tripStore, match: { params: { id } } } = this.props;
     if (!id) {
  ...
  get errorMessages() {
    ...
    let errorMessages = {};
    Object.entries(errors).forEach(([key, value])=> errorMessages[key] = value[0]);
          ...
          <form className={classes.form} onSubmit={this.submit}>
            <FormGroup style={{display: 'inline-block'}}>
              <FormControl
                ...
                <FormHelperText>{this.errorMessages.destination || ''}</FormHelperText>
              </FormControl>
              <MuiPickersUtilsProvider
                libInstance={moment}
                utils={MomentUtils}
            ...
            <FormGroup style={{paddingTop: 50}}>
              <FormControl fullWidth={true} size="medium">
                ...
                <FormHelperText>
                  {this.errorMessages.comment || 'Enter comments about your trip'}
                </FormHelperText>
              </FormControl>
            </FormGroup>
